...
##    │  │   │ ║  ║       functions based on datasheet of MQ135 for adquire   */
##    │╔═╧══╗│ ║  ║       CO2 from the  air quality sensor.                   */
##    │╚╗  ╔╝┘ ║  ║       VERSION 1 BY: G.Krocker                             */
##    └┐║  ╚╗ ╔╝  ║       VERSION 2 BY: Castillo Lorena, Chaparro             */
##     └╚╗  ╚═╝  ╔╝                     Ma. del Pilar & Varón Jelitza         */
##      └╚╗     ╔╝                                                            */
##        ╚═════╝         Bogota, D.C., October 3rd, 2021.                    */
##                                                                            */
...
#Code Reference:https://hackaday.io/project/3475-sniffing-trinket/log/12363-mq135-arduino-library
#               https://github.com/ViliusKraujutis/MQ135
#               https://github.com/balk77/MQ135
#Datasheet:https://www.electronicoscaldas.com/datasheet/MQ-135_Hanwei.pdf
...
class MQ135(object):
    ...
    #adc_value=ADS1015V3.Read_ADC(ADC_channel, PGA, DataRate)
    # Def construct 
    ...
    def get_resistance(self):
        ...
        return ((1023./value)*5. - 1.) * self.RLOAD
    ...
    def get_ppm(self,temperature, humidity):
        """Returns the ppm of CO2 sensed (assuming only CO2 in the air)"""
        return (self.PARA * math.pow((self.get_resistance()/ self.get_corrected_rzero(temperature, humidity)), -self.PARB))

    def get_corrected_ppm(self, temperature, humidity):
        """Returns the ppm of CO2 sensed (assuming only CO2 in the air)
        corrected for temperature/humidity"""
        return (self.PARA * math.pow((self.get_corrected_resistance(temperature, humidity)/ self.get_corrected_rzero(temperature, humidity)), -self.PARB))
